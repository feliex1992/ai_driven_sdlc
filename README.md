...
**STEP 3 — AI CLI** ◑ (partial)

The CLI exists but only exposes `ai context scan` today. The full envisioned
interface (`ai analyze`, `ai design`, `ai plan`, `ai implement`, `ai test`,
`ai security`, `ai review`, `ai deploy`, `ai run <feature>`) is still ahead.

### Still to build

See `ROADMAP.md` for full descriptions. Summary:
